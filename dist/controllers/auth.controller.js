...
const registerTrainer = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const result = yield AuthService.registerTrainer(name, email, password);
...
const registerAdmin = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { name, email, password } = req.body;
        const result = yield AuthService.registerAdmin(name, email, password);
        res.status(201).json({ success: true, message: result.message });
    }
    catch (error) {
        next(error);
...
const registerTrainee = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { name, email, password } = req.body;
        const result = yield AuthService.registerTrainee(name, email, password);
        res.status(201).json({ success: true, message: result.message });
    }
    catch (error) {
        next(error);
